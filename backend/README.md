...
- **Session UseCase + middleware 枠（PR8）**:
  ...
  - **本番 router からは未接続**。dummy token で動く公開 endpoint は作らない。token 交換
    HTTP endpoint は PR9 で Photobook aggregate と一緒に本物の draft_edit_token /
    manage_url_token 検証を組む段階で追加する
  - middleware は失敗時 401 を返し、原因詳細は body / ヘッダに出さない（情報漏洩抑止）
  - raw SessionToken / `session_token_hash` / Cookie 値 / `Set-Cookie` ヘッダ全体は
    ログ・diff・テストログ・レスポンスに出さない（`internal/shared/logging.go` の禁止リスト）
...
→ これらは [`docs/plan/m2-implementation-bootstrap-plan.md`](../docs/plan/m2-implementation-bootstrap-plan.md) §4 / [`docs/plan/m2-session-auth-implementation-plan.md`](../docs/plan/m2-session-auth-implementation-plan.md) §13 の順序で **PR9 以降**に順次追加する。
