...
## PR9c 時点の実装履歴（参考、最新の現在地は新正典を参照）

> 最新の実装状況・PR ロードマップは
> [`docs/plan/vrc-photobook-final-roadmap.md`](../docs/plan/vrc-photobook-final-roadmap.md)
> を参照。本セクションは PR9c 締め時点での履歴であり、その後の実装（Image / Photobook
> 編集 UI / Public Viewer / 管理ページ / Publish / image-processor / Outbox 等）は
> 反映していない。
...
- **Photobook UseCase + 同一 TX 接続（PR9b）**:
  ...
  - HTTP endpoint / 本番 router 接続 / Set-Cookie は PR9c で扱う
...
- **Photobook token 交換 HTTP endpoint（PR9c）**:
  - `internal/photobook/interface/http/`: `Handlers`（`DraftSessionExchange` /
    `ManageSessionExchange`）、Cache-Control: no-store / 固定 401・400 body
  - `internal/photobook/wireup/`: pool + TTL から Handlers を組み立てる facade
    （cmd/api から photobook の internal/usecase を直接 import できないため）
  - `internal/http/router.go`: `POST /api/auth/draft-session-exchange` /
    `POST /api/auth/manage-session-exchange` を本物の token 検証経由のみで接続
  - `cmd/api/main.go`: pool 利用可能時に wireup から Handlers を組み立て、router に渡す
    （pool 未設定時は handler nil で endpoint を生やさない）
  - dummy token / 認証バイパス / 固定 token を一切作らない（grep 確認済）
  - **Set-Cookie は出さない**。raw `session_token` は **成功時 response body にのみ**返し、
    Cookie 化は PR10 の Frontend Route Handler で行う（独自ドメイン取得後）
  - manage session TTL は 7 日（業務知識 v4 §6.15、`cmd/api/main.go` の `manageSessionTTL` 定数）
  - 失敗時 body は `{"status":"unauthorized"}` / `{"status":"bad_request"}` の固定文言で
    原因詳細を出さない

PR9c 時点で未実装だった主要項目（多くは PR10〜PR30 で実装済、最新は新正典参照）:

- 各集約 DDL / sqlc query / Repository / UseCase / Handler（Image / image_variants /
  upload_verification_sessions / outbox_events / photobook_ogp_images 等は実装済。
  reports / usage_windows / manage_url_deliveries / moderation_actions はまだ未実装）
- CORS / Auth middleware（接続済）
- R2 / Turnstile（接続済）/ email provider（ADR-0006 で再選定中）
- Outbox table + 同 TX INSERT（実装済）/ outbox-worker CLI も実装済。Cloud Run Jobs /
  Scheduler 化は未実施（image に同梱、ローカル CLI / Cloud SQL Auth Proxy 経由で実行）
- Cloud Run deploy / Cloud SQL（PR12 / PR16 で運用開始済）
...
- R2 / Turnstile は接続済（cmd/api 起動時に Secret 注入があれば handler を生やす）。
  email provider は ADR-0006 で再選定中で **未接続**
...
- 本番 DSN は Secret Manager から Cloud Run env vars 経由で注入する（運用中、PR16 以降）
...
sqlc は集約別に分割済（auth/session / photobook / image / uploadverification / outbox /
最小 health の 6 set、`backend/sqlc.yaml` 参照）。
