...
- **Photobook 集約 domain + repository（PR9a）**:
  ...
  - migration: `00003_create_photobooks.sql` + `00004_add_photobooks_fk_to_sessions.sql`
    （PR7 で意図的に保留した `sessions.photobook_id` への FK を本 PR で追加）
  - 状態遷移メソッドは domain 側のみ実装（NewDraftPhotobook / Publish / ReissueManageUrl /
    TouchDraft）、**UseCase / Session 連携 / HTTP endpoint / 本番 router 接続は PR9b/9c**
  - すべての UPDATE 系 query は `version = $expectedVersion` を WHERE に含む（楽観ロック、
    0 行 UPDATE で `ErrOptimisticLockConflict`）
  - draft / published 状態整合性を CHECK 制約で保証（draft_edit_token_hash / draft_expires_at /
    public_url_slug / manage_url_token_hash / published_at の組合せ）
  - raw `draft_edit_token` / `manage_url_token` / hash・Cookie・Set-Cookie 全体は
    ログ・diff・テストログに出さない（VO に `String()` 非実装、`Reveal()` 明示呼び出しのみ）
...
→ これらは [`docs/plan/m2-implementation-bootstrap-plan.md`](../docs/plan/m2-implementation-bootstrap-plan.md) §4 / [`docs/plan/m2-session-auth-implementation-plan.md`](../docs/plan/m2-session-auth-implementation-plan.md) §13 / [`docs/plan/m2-photobook-session-integration-plan.md`](../docs/plan/m2-photobook-session-integration-plan.md) §13 の順序で **PR9b 以降**に順次追加する（PR9b: UseCase + Session revoke 同一 TX 接続 / PR9c: HTTP endpoint 接続）。
