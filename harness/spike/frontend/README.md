...
3. ~~**Backend PoC（基盤）**~~ ✅ 2026-04-25 完了
4. ~~**R2 接続 PoC（CLI 範囲）**~~ ✅ 2026-04-25 完了。実 R2 接続は `harness/spike/backend/README.md` 検証手順でユーザー側実施
5. ~~**Frontend / Backend 結合検証 PoC（CLI 範囲）**~~ ✅ 2026-04-25 完了（下記「結合検証 PoC」参照）
6. **Cloudflare Workers + Cloud Run 実環境デプロイ + 結合検証**（次）
7. Turnstile / Outbox / Email Provider PoC

## 結合検証 PoC

`/integration/backend-check` ページで Backend `/sandbox/session-check` / `/sandbox/origin-check` を叩いて、CORS / Cookie / Origin 動作を画面上で確認できる。

### 起動

```sh
# Backend（別ターミナル）
PORT=8090 ALLOWED_ORIGINS='http://localhost:8787' /tmp/spike-api

# Frontend
echo "NEXT_PUBLIC_API_BASE_URL=http://localhost:8090" > harness/spike/frontend/.env.local
npm --prefix harness/spike/frontend run cf:build
npm --prefix harness/spike/frontend run cf:preview
# → http://localhost:8787/integration/backend-check
```

### 確認すること

- `GET /healthz` ボタン: 200 が返る
- `GET /sandbox/session-check (credentials: include)` ボタン: ブラウザが **同オリジン Cookie のみ送る**ため、Backend が別オリジンなら `false` / `false`
- `POST /sandbox/origin-check (credentials: include)`: preflight + メイン POST が成立、200 `{"origin_allowed": true}`
- DevTools / Web Inspector の Network タブで `Access-Control-Allow-Origin: http://localhost:8787` が反射されている

### Cookie 引き渡しの限界

Frontend (`localhost:8787`) と Backend (`localhost:8090`) はポート違いで **別オリジン**。Cookie Domain 未指定では引き渡せない（U2、ADR-0003）。

実環境では共通親ドメイン下または Backend を Frontend 経由でプロキシする構成で再確認する。詳細は `harness/spike/backend/README.md` §「Cookie 引き渡しの限界（重要）」を参照。
