...
│   ├── db/
│   │   ├── pool.go                   # pgx の最小プール
│   │   ├── queries/                  # sqlc 生成元クエリ（test_alive / upload_verification）
...
1. **R2 Wrangler 実操作疎通検証**（次工程）
   - ユーザー: `npx wrangler login`
   - Claude Code: `wrangler whoami` / `r2 bucket list` / 必要なら `r2 bucket create vrcpb-spike` / `r2 object put` / `r2 object get` / 元ファイルとの一致確認 / `r2 object delete`
   - **API Token / Access Key はこの段階では扱わない**（Wrangler 経由の Cloudflare 認証のみ）
2. **R2 S3 API + presigned URL 実接続検証**
   - 1 が成立した後、別工程で R2 API Token を発行（Object Read & Write、対象バケット限定、短期有効）
   - 本書「R2 接続 PoC 検証手順」の curl A〜H を実 Cloudflare アカウントで通す
3. **Cloud Run + Cloudflare Workers 実環境デプロイ**
   - Cookie Domain（U2、ADR-0003）、CORS、Origin、SameSite=Strict をエンドツーエンドで確認
4. **Turnstile 本番 widget 発行**（Workers の hostname 確定後）
   - Cloudflare Dashboard → Turnstile → Add widget で発行。本番 secret は `.env.local` または Secret Manager のみで扱う（チャット・ログに貼らない）
5. **Outbox / 自動 reconciler 起動基盤 PoC**（M1 優先順位 7）
6. **Email Provider 選定**（ADR-0004 Proposed → Accepted、M1 優先順位 8）
