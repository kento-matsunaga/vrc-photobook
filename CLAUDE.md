...
→ 公開 Viewer → OGP 自動生成 → 公開配信 + 通報受付 + 運営 hide/unhide + RateLimit** までの
主要動線が本番経路で稼働中（PR12〜PR36 完了）。
...
- outbox-worker（CLI）+ image 同梱 + Cloud Run Job `vrcpb-outbox-worker` + Cloud Scheduler
  `vrcpb-outbox-worker-tick` ENABLED（`* * * * *` / Asia/Tokyo、ADR-0007 publish 同期 OGP
  の fallback、運用は `docs/runbook/outbox-worker-ops.md`）
- OGP 独立管理（`photobook_ogp_images` + Cloudflare Workers proxy + R2 binding、
  R2 public OFF を維持）+ publish 同期生成（commit 後 best-effort 2.5s timeout、ADR-0007）
  + `photobook.published` outbox handler 連携
- Moderation（hide / unhide）+ `cmd/ops photobook` 系（PR34）
- Report 集約（通報受付 + Outbox + Turnstile L0-L4 多層ガード + Safari 実機 smoke、PR35b / PR36-0）
- UsageLimit 集約（report.submit / upload_verification.issue / publish.from_draft の固定窓 RateLimit + cmd/ops usage list/show + Safari 実機 429 smoke、PR36）
...
- Moderation 拡張（softDelete / restore / purge / reissueManageUrl）→ 後続 PR
- SubmitReport の visibility 要件再判断（PR36 STOP ε で発見、unlisted も対象にするか）→ 後続 PR
