...
M1 ローカル PoC で成立した構成（Frontend / Backend / R2 / Turnstile / Outbox）を**実環境**で再現し、以下の前提が崩れないことを確認する。崩れた場合は本書 §12「失敗時の判断」で代替案へ切り替え、ADR を更新する。
...
- **Backend**: `harness/spike/backend/` を multi-stage Dockerfile（`golang:1.24-alpine` → `gcr.io/distroless/static-debian12:nonroot`、PoC で 12.4MB / 21.6MB 確認済み）でビルド → Artifact Registry → Cloud Run（`asia-northeast1`、東京リージョン）。
  - **イメージ構成（M1 第一案）**: 同一 Docker image に `cmd/api` と `cmd/outbox-worker` の 2 バイナリを含める。Cloud Run service は `api` を `--command` で起動、Cloud Run Jobs は `outbox-worker` を `--command` で起動。詳細は §5 Step 11.0 を参照。
- **DB**: Cloud SQL PostgreSQL 16（検証用、最小サイズ）。Cloud SQL Auth Proxy 経由 / Public IP + Cloud SQL connector のいずれかを採用（§5 デプロイ順序で決める）。
  - **代替**: Cloud Run コンテナ内 sqlite を使わず PostgreSQL を保つ。Cloud SQL の起動コストが M1 検証に対して過剰な場合のみ、Compute Engine 上の単一インスタンス PostgreSQL を一時 DB として代替（§12 で扱う）。
...
| **Secret Manager** | R2 / DB / Turnstile Secret の格納 | §4 のキー一覧に従って登録 |
...
- 検証完了後、API Token は **Revoke**、検証用 Secret は Secret Manager から削除する（§13 後片付け手順）。

---

## 5. デプロイ順序

**重要**: 各ステップで失敗 / 仕様差異が見つかったら即停止し、§12 で代替案を検討する。「動くまで雑に進める」のは禁止。
...
- **代替**: Cloud SQL の起動コストが過剰な場合、Compute Engine `e2-micro` に PostgreSQL 16 を一時的に立てる（§12 で詳細）。

### Step 4. Secret Manager 登録

- §4 一覧の Secret を登録（**M1 では SendGrid は登録しない**）。
...
§7 のチェックリストを **macOS Safari 最新 + iPhone Safari 最新** で実施。`.agents/rules/safari-verification.md` の必須項目を踏襲。
...
- ADR-0003 §13 U2 に Cookie Domain 検証結果（§6 のどの案を採るか）を追記。
...
### 7.2 iPhone Safari（最新、可能なら 1 世代前も）
...
## 9. Outbox / Cloud Run Jobs 確認

### 9.1 Cloud Run Jobs での `outbox-worker --once` 起動

> **前提**: §5 Step 11.0 のとおり、M1 第一案として **同一 Docker image に `api` / `outbox-worker` の 2 バイナリを含める**方針を採用する。Cloud Run service と Cloud Run Jobs はどちらも同じイメージを参照し、`--command` で起動するバイナリを切り替える。
...
### 10.2 M2 早期で実施する SendGrid PoC（参照: ADR-0004 §M2 以降の TODO）
...
### 10.3 SendGrid アカウント審査が落ちた場合のフォールバック
...
以下を**全て満たした時点で M1 実環境デプロイ検証は完了**とする。1 つでも満たさない場合は §12 失敗時の判断へ。
...
| 12 | §13 後片付け手順が実行可能な状態である（`.workers.dev` / Cloud Run / Cloud Run Jobs / Cloud Scheduler / Cloud SQL / Artifact Registry / Secret Manager / R2 のリソースを停止・削除する手順がメモされている） |
...
- [ ] Cloud Run service `vrcpb-spike-api` を停止（min-instances=0 にしておけば追加対応不要だが、不要なら削除）
- [ ] Cloud Run Jobs `vrcpb-spike-outbox-worker` を削除
- [ ] Cloud Scheduler `vrcpb-spike-outbox-trigger` を削除（残すと毎 10 分課金）
- [ ] Artifact Registry の検証イメージ（`api:m1-live` 等）を削除
- [ ] Cloud SQL `vrcpb-spike-pg` インスタンスを停止（`gcloud sql instances patch ... --activation-policy=NEVER`）または削除
...
| 2026-04-26 | レビュー反映：(1) Cloud Run Jobs の outbox-worker 起動方式を「同一 Docker image に api/outbox-worker の 2 バイナリを含めて --command で切替」を M1 第一案として §2 / §5 Step 11.0 / §9.1 に明記。Dockerfile の最小修正が前提であることも追記。(2) Turnstile 本番 widget は M1 では原則作成しない方針を §2 / §3.1 / §4 / §13.1 で再強調。本番 widget は M2 早期タスクへ |
