...
| **Q-G** | Viewer PC sticky right panel | **design 通り sticky right panel を新規 component として追加**。Creator card / Report link / OGP annotation の visual を archive に合わせる。production data で足りない項目は mock せず明記。**ただし OGP annotation (`/ogp/{photobookId}?v=1` 表記) は raw photobook_id が UI に露出するため production では非表示** (β-5 判断、§安全側方針)。OGP 画像配信自体は metadata `og:image` で機能、UI annotation 行のみ skip | design archive: `wf-screens-c.jsx:55-70` PC right panel（Creator card 36×36 avatar + name + @x_id / Report link / OGP annotation）。既存 public viewer の data: creator_display_name / cover variants / pages / photos は API response に存在。X ID / avatar URL は **API response にない可能性** → STOP β-5 で実装前に確認、不足なら「avatar 部分は dashed empty / X ID なし」表示で対応。creator_display_name 空の published photobook (a8fe0db hotfix 後あり得る) は「作者未設定」fallback で表示 (β-5 linter 補正反映) |
