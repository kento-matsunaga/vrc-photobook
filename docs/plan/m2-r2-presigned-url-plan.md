...
[Step C] STOP: ユーザー手動 GCP Secret Manager 登録
   ...
   - 既存 R2_* (spike 用) がある場合は spike service を `R2_*:1` で pin → versions add
   ↓
[Step D] STOP: Cloud Run revision 更新（env 注入）
   - `gcloud run services update vrcpb-api --update-secrets=R2_*:latest,TURNSTILE_SECRET_KEY:latest`
   - new revision にトラフィック 100%
   ↓
[Step D.5] STOP: Backend image rebuild + push + Cloud Run image 更新（2026-04-27 Audit 追加）
   - PR12 で deploy した :500f8cc image は PR18〜PR21 Step A の Backend code を含まない
   - HEAD commit short hash で tag を付けた image を rebuild + Artifact Registry へ push
   - `gcloud run services update vrcpb-api --image=...:<commit>` で revision の image を差し替え
   - 既存 env 注入（R2_* / TURNSTILE_SECRET_KEY / DATABASE_URL）は維持される
   - 旧 image (`:500f8cc` 等) は Artifact Registry に残し rollback 用とする
   - 実行コマンド例:
     ```sh
     COMMIT=$(git rev-parse --short HEAD)
     IMAGE=asia-northeast1-docker.pkg.dev/project-1c310480-335c-4365-8a8/vrcpb/vrcpb-api:$COMMIT
     ...
     ```
   - 確認項目:
     - 起動 log に `r2 configured; image upload endpoints enabled`
     - `/health` 200 / `/readyz` 200
     - dummy POST `/api/photobooks/{id}/images/upload-intent` (Cookie 無し) → **401**（404 ではない = endpoint 登録確認）
   - 注意: env 注入だけ更新しても image が古いと code は反映されない。
     `--update-secrets` と `--image` の両方を 1 サイクルで実行する運用に揃える。
   ↓
...
| C | Secret 登録（spike 既存時は spike を `:1` pin → versions add）| `gcloud secrets versions list` で `enabled` 確認、値は確認しない |
| D | revision 更新（env 注入） | `gcloud run services describe vrcpb-api --format=...` で env 注入確認 |
| D.5 | image rebuild + push + image 更新 | image tag が新 commit / 起動 log に `r2 configured` / dummy POST 401 |
| E | curl 確認 | Claude Code が指示出し、出力解釈 |
