...
| **`vrcpb-outbox-worker-tick` Cloud Scheduler 新規作成** | `* * * * *` ENABLED、OAuth 認証 (compute SA)、image-processor-tick と同パターン |
...
#### STOP γ receipt (2026-05-11 実施)

| 項目 | 結果 |
|---|---|
| Cloud Build submit | SUCCESS、build duration ~3m42s。新 image tag 反映済 |
| Service revision 切替 | 新 revision が `latestRevision=true / percent=100` で配信、`predeploy-verification-checklist.md` §1 / §2 通過 |
| Cloud Run Jobs image tag 同期 | `vrcpb-image-processor` / `vrcpb-outbox-worker` の両 Job を新 image に更新済 |
| `vrcpb-outbox-worker-tick` 作成 | `state=ENABLED` / `schedule='* * * * *'` / `timeZone=Asia/Tokyo` / OAuth `serviceAccountEmail=271979922385-compute@developer.gserviceaccount.com` / `attemptDeadline=120s` / `max-retry-attempts=0`、image-processor-tick と同パターン |
| Backend smoke A〜E | 全 PASS。`/health` / `/readyz` 200、`/api/public/photobooks/<dummy>` 404 JSON、PATCH / DELETE preflight 200、`Access-Control-Allow-*` 既存維持、認証なし publish 401 |
| 新 revision wireup ログ | `publish: OGP sync enabled (commit-after best-effort + outbox fallback)` 出力を確認（M-2 配線済） |
| outbox-worker 起動ログ | `outbox-worker: ogp generator wired (photobook.published will trigger OGP generation)` を Job 起動時に確認 |
| `event=ogp_sync_result` ログ | 観測ウィンドウ内では 0 件（実 publish 無し）。**STOP δ の実 publish smoke で初観測**する想定で扱う |
| Secret 漏洩 grep | Cloud Build logs / 新 revision logs (30 分) / outbox-worker Job logs (5 分) すべて hit 0 件 |
| Scheduler 起動観測 | 初回 2 attempts は HTTP 403（後述 IAM 修復前）、IAM 付与後の 3 attempts 連続 HTTP 200。Job executions も対応 3 件すべて `succeededCount=1` で完了、Container exit(0)、worker は idle で正常終了 |
| **IAM 修復 (γ-fix)** | `vrcpb-outbox-worker` Job に compute SA の `roles/run.invoker` が **付与されておらず**、Scheduler 初回 attempt が PERMISSION_DENIED で 403 返却。`gcloud run jobs add-iam-policy-binding` で付与後即復旧。詳細は `harness/failure-log/2026-05-11_outbox-worker-job-iam-missing-on-creation.md` |
| Rollback target | 直前 Service revision / 旧 image tag を deploy 前に控え済（Scheduler は新規作成のため `delete` で巻戻し可、IAM 付与は冪等で rollback 不要） |
| runbook 作成 | `docs/runbook/outbox-worker-ops.md` を本 commit で起票 |
...
| 2026-05-11 | STOP β commit + push（Backend OGP 同期化実装）。`feat(ogp): wire publish sync OGP generation with outbox fallback` |
| 2026-05-11 | STOP γ 実施完了。Backend deploy / Cloud Run Jobs image 同期 / Scheduler `vrcpb-outbox-worker-tick` ENABLED / IAM 修復（compute SA に `roles/run.invoker` 付与）/ smoke A〜E 全 PASS。STOP γ receipt 表を追記、関連 failure-log + runbook 起票 |
