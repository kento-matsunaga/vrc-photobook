...
- **第一候補は `vrcphotobook.com` に確定**（ユーザー判断、2026-04-26）
...
- Frontend Workers: 引き続き `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev` で運用
- Backend Cloud Run: 引き続き `https://vrcpb-spike-api-7eosr3jcfa-an.a.run.app` で運用
- U2 別オリジン Cookie 不通の状態は **想定通り** として継続（M1 完了判定でも合意済）
- 24h / 7 日後 Safari ITP 観察は継続（起点 2026-04-26）

---

## 10. 次にユーザーが判断すべきこと（再掲）

§9.3 の購入解禁の判断基準が満たされたタイミングで、以下を確定して購入手続きに入る:

1. **どのドメインを買うか** — 第一候補 `vrcphotobook.com`（合意済、§9.1）。`.com` で取れない場合は第二候補 `vrcphotobook.app`
2. **取得元** — Cloudflare Registrar（合意済、§8）
3. **購入タイミング** — §9.3 の判断基準のいずれかが満たされた時点
4. **購入後すぐ** — `app.vrcphotobook.com` Workers Custom Domain / `api.vrcphotobook.com` Cloud Run Domain Mapping / Cookie Domain `.vrcphotobook.com` 切替（[`m2-early-domain-and-cookie-plan.md`](./m2-early-domain-and-cookie-plan.md) §8 切替手順に従う）
