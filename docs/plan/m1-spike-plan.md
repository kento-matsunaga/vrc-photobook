...
| フィードバック項目 | 想定更新箇所 | 状況 |
|------------------|------------|------|
| R2 content-length-range 実機検証結果（U7 関連） | §Content-Length 検証 | M1 R2 接続 PoC（実 R2 接続）で確認予定 |
...
- [x] **R2 接続 PoC（aws-sdk-go-v2 / presigned PUT / HeadObject）のコード実装完了**（2026-04-25、コミット `a33be4c`）。実 R2 接続検証はユーザー側 Wrangler 実操作 → S3 API 接続の順で進行
...
7. **R2 Wrangler 実操作疎通検証** ← **次工程**
   - ユーザーが `npx wrangler login` を実施
   - Claude Code は `wrangler whoami` / `r2 bucket list` / `r2 bucket create vrcpb-spike` / `r2 object put` / `r2 object get` / 元ファイルとの一致確認 / `r2 object delete` を実行
   - **API Token / Access Key はこの段階では扱わない**（Wrangler 経由の Cloudflare 認証のみ）
   - Wrangler のバージョンによっては `r2 object list` が無いことがあるため、`bucket list` と `object get/put/delete` を主に使う
8. **R2 S3 API + presigned URL の実接続検証**
   - 7 の疎通が取れた後、別工程で R2 API Token を発行（Object Read & Write、対象バケット限定、短期有効）
   - Go backend の `/sandbox/r2-*` エンドポイントを実 Cloudflare 接続で動作確認
9. **Cloudflare Workers + Cloud Run 実環境デプロイ検証**
   - Frontend → `*.workers.dev`、Backend → `*.run.app`
   - Safari 実機での再確認、24h / 7 日後の ITP 影響観察開始
10. **Turnstile 本番 widget 発行**
...
11. **Outbox / 自動 reconciler 起動基盤 PoC**（優先順位 7）
12. **Email Provider 選定検証**（優先順位 8、ADR-0004 Proposed → Accepted）
