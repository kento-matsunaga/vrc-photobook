...
Frontend PoC + Safari 実機検証 + Backend PoC（CLI / Docker）が完了したため、進行順序を以下に更新:

1. ~~**macOS Safari 実機検証**~~ ✅ 2026-04-25 完了
2. ~~**iPhone Safari 実機検証**~~ ✅ 2026-04-25 完了
3. ~~**Backend PoC（Cloud Run + Go chi + pgx + sqlc + goose）**~~ ✅ 2026-04-25 完了
4. **R2 接続 PoC（優先順位 4）** ← **次工程**
   - Cloud Run 相当の Go バックエンドから R2 へ接続できるか
   - presigned URL（PUT、15 分有効）を発行できるか
   - HeadBucket / ListObjects / HeadObject / 10MB 検証
   - **HEIC 変換 / libheif は本 PoC に含めない**（重い検討のため別 PoC または未解決事項として保留）
5. Frontend / Backend 結合検証
   - Cookie Domain（U2）/ CORS / Origin / SameSite=Strict のエンドツーエンド確認
6. Cloudflare Workers + Cloud Run 実環境デプロイ検証
   - Frontend → `*.workers.dev`、Backend → `*.run.app`
   - Safari 実機での再確認、24h / 7 日後の ITP 影響観察開始
7. **Turnstile + upload_verification_session PoC**（優先順位 5）
8. **Outbox / 自動 reconciler 起動基盤 PoC**（優先順位 7）
9. **Email Provider 選定検証**（優先順位 8、ADR-0004 Proposed → Accepted）
