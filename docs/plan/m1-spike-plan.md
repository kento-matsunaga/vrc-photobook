...
12. **後続候補（M2 早期）**
    - C+D 既知問題の修正：`metadataBase` 設定（OGP `og:image` の絶対 URL 解決）+ middleware 一本化（X-Robots-Tag 二重出力解消）
    - 独自ドメイン取得 + Cookie Domain 案 A 採用（U2 確定）
    - Cloud SQL Step B 起動（Outbox / Cloud Run Jobs / Cloud Scheduler / U11 検証）
    - SendGrid 実送信 PoC（ADR-0004 §M2 以降の TODO）
