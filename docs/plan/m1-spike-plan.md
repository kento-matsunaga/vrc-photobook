...
ADR-0001 で Frontend Deploy 第一候補として Cloudflare Pages を選定。理由はエッジ配信・無料枠・R2 との親和性。一方で Next.js 15 App Router の Cloudflare Pages 対応は OpenNext / `@cloudflare/next-on-pages` の 2 系統があり、いずれも実戦事例が少ない。
...
- [ ] §11 の Safari / iPhone Safari 検証項目を実機で完了
