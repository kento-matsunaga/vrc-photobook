...
> **2026-04-26: M1 完了承認済**。詳細は §13.4 と `harness/work-logs/2026-04-26_m1-completion-judgment.md`。残項目（Cloud SQL / Cloud Run Jobs / Cloud Scheduler / SendGrid 実送信 / Turnstile 本番 widget / U2 独自ドメイン）は M2 早期に持ち越し、24h / 7 日後 Safari ITP は継続観察として M1 完了をブロックしない方針で合意。

### 13.1 必須

- [x] §5 の各成功条件をすべて満たす（C+D 修正で既知問題 og:image / X-Robots-Tag 二重出力を解消、コミット `afa3c7b`）
...
- [x] **Cloudflare Workers 実環境（`*.workers.dev`）での Frontend PoC デプロイ動作確認**（2026-04-26 完了。URL: `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev`、`harness/work-logs/2026-04-26_m1-live-deploy-verification.md`）
- [x] **Cloud Run 実環境（`*.run.app`）での Backend PoC デプロイ動作確認**（2026-04-26 完了。URL: `https://vrcpb-spike-api-7eosr3jcfa-an.a.run.app`、`/health` 200 / `/readyz` 503 db_not_configured / R2 系すべて 200 / CORS / Origin 結合確認 / Cloud Logging slog JSON / Secret 漏洩 0。Cloud SQL は §4.4 段階化により未起動、コールドスタート / SIGTERM 詳細計測は M2 早期に持ち越し）
- [-] **R2 Wrangler 実操作疎通検証**: Wrangler 4.x の OAuth スコープに `r2:*` が無いため中断扱い。本来の検証目的（Go backend → R2）は S3 API 経由で達成済（`reference_wrangler_oauth_no_r2.md` メモリ参照）
- [x] §12 のフィードバック項目を ADR / 設計書に反映（ADR-0001 / ADR-0003 / ADR-0004 / ADR-0005 / 計画書すべて反映、コミット `e8f7029` / `6e38354` / `afa3c7b`）
- [x] U5（アトミック消費）解消（2026-04-25 Turnstile PoC で確定）
- [-] U2（Cookie Domain）/ U7（storage_key 乱数）/ U11（起動基盤）の 3 つを最終確定: U5 は M1 で解消、**U2 は別オリジン Cookie 不通を確定材料として記録 + M2 早期に独自ドメイン取得 → 案 A 採用を一次方針に確定**（ADR-0003 §M1 検証結果）、U7 / U11 は M2 早期持ち越し
...
- [x] PoC コード（`harness/spike/`）が動作する状態でリポジトリにコミット済み（本実装と分離）。Frontend Workers / Backend Cloud Run の実環境動作を 2026-04-26 に確認、最新コミットは `afa3c7b`
- [x] **M1 実環境デプロイ前に GCP Budget Alert を設定**（1,000 円、ユーザー側で 2026-04-26 に設定確認、`harness/work-logs/2026-04-26_m1-completion-judgment.md` §1.8）
- [x] **Cloud SQL は M1 実環境デプロイで「最初から常時起動」を避け、必要時のみ短時間起動する方針を採用**（同 §4.4 段階化 Step A → B → C）。**M1 では Cloud SQL を起動せず Step A だけで完結**、Step B / C は M2 早期に持ち越し
- [-] **M1 実環境デプロイ完了時の後片付け**: 完了判定表 §4 後片付け判断に従い、**M2 早期で再利用するため Cloud Run / Workers / image / Secret は当面残置**。M2 完了後にまとめて削除予定（Cloud SQL は未作成のため停止対象なし、Scheduler / Jobs も未作成）
...
11. ~~**Cloudflare Workers + Cloud Run 実環境デプロイ検証**~~ ✅ **完了**（2026-04-26、Step 7〜10、`harness/work-logs/2026-04-26_m1-live-deploy-verification.md`）
    - Frontend → `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev`、Backend → `https://vrcpb-spike-api-7eosr3jcfa-an.a.run.app`
    - macOS Safari / iPhone Safari 実機確認すべて成立、24h / 7 日後の ITP 影響観察は **起点 2026-04-26**で継続観察
    - **Cookie Domain U2**: 別オリジン構成で `session-check` が `false / false` になる挙動を確認 → ADR-0003 §13 U2 で「**M2 早期に独自ドメイン取得 → 案 A 採用**」を一次方針として確定
    - **Cloud Run Jobs + Cloud Scheduler / U11**: 未着手。Cloud SQL 起動を伴うため計画書 §4.4 段階化 Step B 以降（M2 早期）
    - **Cloud Run 東京 ↔ R2 レイテンシ計測**: M1 で R2 系 200 応答は確認済、p50 詳細計測は M2 早期で
    - **Turnstile 本番 widget**: Workers hostname (`vrcpb-spike-frontend.k-matsunaga-biz.workers.dev`) 確定済、必要時に Cloudflare Dashboard → Turnstile → Add widget で発行（M2 早期）
12. ~~**C+D 既知問題の修正**~~ ✅ **完了**（2026-04-26、コミット `afa3c7b`）
    - `metadataBase` 設定で OGP `og:image` の絶対 URL 解決（`http://localhost:3000` → `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev/og-sample.png`）
    - `next.config.mjs` の `headers()` を削除し middleware 一本化、`X-Robots-Tag` 二重出力解消
13. **後続候補（M2 早期、優先順位は `harness/work-logs/2026-04-26_m1-completion-judgment.md` §3）**
    - **優先度 A**: 独自ドメイン取得 + Cloudflare DNS / Workers Custom Domain + Backend API ドメイン方針 + U2 Cookie Domain 案 A 採用 + 本実装移植計画
    - **優先度 B**: Cloud SQL Step B 起動 → migration → DATABASE_URL Secret → Cloud Run Jobs / Cloud Scheduler → U11 解消 → Cloud SQL Step C 停止 / 削除
    - **優先度 C**: SendGrid 実送信 PoC（ADR-0004 §M2 以降 TODO）/ Turnstile 本番 widget 発行 / 24h / 7 日後 Safari ITP 観察結果反映
...
- [ ] iPhone Safari で **24 時間後 / 7 日後の Cookie 残存**（ITP 長期影響評価、起点 **2026-04-26** Workers 実環境 `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev`）
- [ ] iOS Safari 1 世代前 / iPad Safari / プライベートブラウジング動作
- [x] Cloudflare Workers 実環境デプロイ後の Safari 再確認（2026-04-26 完了、大きな問題なし）
...
→ **2026-04-26 時点でいずれも発生せず**。Frontend は Cloudflare Workers + Static Assets binding（OpenNext）で成立、Backend は Cloud Run で成立、token → session 交換方式は macOS Safari / iPhone Safari 実機で成立、R2 presigned URL は実 R2 + Cloud Run 経由で成立を確認。
...
|---|---|
| **M1 完了日** | **2026-04-26** |
| **承認** | ユーザー承認済（2026-04-26、本日のセッション内）|
| **完了判定の根拠** | `harness/work-logs/2026-04-26_m1-completion-judgment.md`（M1 完了判定表、コミット `256e3d1`）|
| **実環境デプロイ完了ログ** | `harness/work-logs/2026-04-26_m1-live-deploy-verification.md`（コミット `6e38354`）|
| **Frontend Workers URL（M2 早期まで保持）** | `https://vrcpb-spike-frontend.k-matsunaga-biz.workers.dev` |
| **Backend Cloud Run URL（同上）** | `https://vrcpb-spike-api-7eosr3jcfa-an.a.run.app` |

#### 完了に伴う合意事項

1. **M2 早期持ち越し項目**（M1 完了をブロックしない）:
   - Cloud SQL 実環境作成 / Cloud Run Jobs / Cloud Scheduler（計画書 §4.4 段階化通り）
   - Turnstile 本番 widget 作成
   - SendGrid 実送信 PoC（ADR-0004 §M2 以降 TODO）
   - U2 独自ドメイン取得 + Cookie Domain 案 A 採用
   - 本実装ディレクトリ `frontend/` / `backend/` への移植
2. **継続観察項目**（M1 完了をブロックしない、時間依存）:
   - 24h / 7 日後 Safari ITP（起点 2026-04-26、`.agents/rules/safari-verification.md` §履歴）
   - iOS Safari 1 世代前 / iPad Safari / プライベートブラウジング
3. **後片付け方針**:
   - **M1 完了直後の削除は実施しない**。M2 早期に Frontend Workers / Backend Cloud Run / Artifact Registry image / Secret Manager R2_* / Budget Alert を再利用
   - Cloud SQL / Jobs / Scheduler は **未作成のため後片付け対象なし**
   - 月額予測 約 $0.30〜数十円、Budget Alert 1,000 円に十分余裕
4. **次工程**:
   - 優先度 A の独自ドメイン取得 + U2 Cookie Domain から着手することを一次推奨（完了判定表 §3 参照）
