...
| - | テスト送信 1 通が成功し、送信履歴 UI / API ログの本文表示を実機確認 | （**未実施**） M2 早期に AWS SES sandbox 解除 + 1 通テスト送信 + bounce/complaint webhook 受信を別タスクで実施。本 ADR の Accepted は「provider 選定として」 |
| ✅ | 第一候補が決まり、Accepted 化の条件が ADR-0004 に書ける状態になる | **達成**：第一候補 AWS SES（Tokyo region・本文非保持・event payload に body なし・MVP 安価）、第二候補 SendGrid（公式が「本文を送信中だけ保持」と明記）、不採用 Resend / Mailgun / Postmark / Cloudflare Email Service（理由は ADR-0004 に詳述） |
...
| 第一候補確定 → Proposed → Accepted | §ステータス + §決定 | **2026-04-25 Accepted（MVP プロバイダ選定として）**：第一候補 AWS SES、第二候補 SendGrid、不採用 Resend / Mailgun / Postmark / Cloudflare Email Service |
| 実装方針の整理 | §実装方針 | **2026-04-25 完了**：EmailSender ポート抽象化、本文最小化、Outbox payload に管理 URL を入れない、subject に token を含めない、recipient_email 24h NULL 化、application ログには message_id のみ |
| 実送信 PoC（1 通テスト + bounce 受信） | §M2 以降の TODO | **未実施**。M2 早期に sandbox 解除申請 + テスト送信 + bounce/complaint 受信を別タスクで実施 |
...
| ADR-0004 | **Accepted（MVP プロバイダ選定として、2026-04-25）** | M2 早期に実送信 PoC 完了で「Accepted」に注記を外す |
...
- [x] **ADR-0004 を Accepted 化**（2026-04-25、4 候補比較完了 + 第一/第二候補確定 + 実装方針整理。実送信 PoC は M2 早期に別タスクで実施する旨を明記）
...
10. ~~**Email Provider 選定検証**~~ ✅ **完了（書類調査ベース、ADR-0004 Accepted）**（2026-04-25、優先順位 8）
    - 4 候補 + 参考 2 候補（Postmark / Cloudflare Email Service）の公式ドキュメントから dashboard 本文表示・retention・event payload・地域・無料枠を比較表化
    - 第一候補: AWS SES（Tokyo region・本文非保持・event payload に body なし・MVP 安価）
    - 第二候補: SendGrid（公式が「本文を送信中だけ保持」と明記、米国データセンターのみ）
    - 不採用: Resend（dashboard で本文表示 + sensitive 非保存は厳条件 add-on） / Mailgun（retention 中本文保存） / Postmark（45 日 default 本文保存） / Cloudflare Email Service（public beta、limit 変動）
    - 残: M2 早期に AWS SES sandbox 解除 + 1 通テスト送信 + bounce/complaint webhook 受信 + Cloud Run からの実送信レイテンシ計測
