...
### 11.3 smoke target は visibility=public 必須

SubmitReport は `visibility='public'` を厳格要求する（`backend/internal/report/internal/usecase/submit_report.go:163`）。MVP 既定値の `unlisted` photobook は通報不可（`ErrTargetNotEligibleForReport` → Frontend `not_found`）。
