...
## 11. 実機 429 smoke 注意（PR36 STOP ε で得た教訓）

本番 DB に最小副作用で 429 UI を実機確認するための注意点。詳細経緯:
[`harness/work-logs/2026-04-30_pr36-usage-limit-result.md`](../../harness/work-logs/2026-04-30_pr36-usage-limit-result.md) §13 /
[`harness/failure-log/2026-04-30_usage-limit-smoke-window-rollover.md`](../../harness/failure-log/2026-04-30_usage-limit-smoke-window-rollover.md)。

### 11.1 固定窓 rollover に注意

`report.submit` の狭粒度（5 分窓）counter のみを `count=limit` に UPDATE して 429 を作る方法は、
**ユーザー操作との時間ズレで window rollover** すると counter が新窓 `count=0` で再開し、429 にならず通常 submit が成立する。

→ 必ず**狭粒度 current 窓 + 広粒度（1 時間窓）の両方** を threshold 化する。広粒度は rollover しにくく、狭粒度が rollover しても広粒度で必ず 429 を拾う。

### 11.2 current window はサーバ now 基準で計算

クライアント / Python ローカル時刻ではなく、PostgreSQL `now() AT TIME ZONE 'UTC'` を取得して `floor(now/window_seconds) * window_seconds` で window_start を計算する。

### 11.3 smoke target は visibility != private 必須

SubmitReport は **`visibility != 'private'` AND `hidden_by_operator=false` AND `status='published'`** を要求する（`backend/internal/report/internal/usecase/submit_report.go` の `assessReportEligibility`、PR36 後の案 B）。`public` / `unlisted` の photobook が smoke target になる。
`private` / `hidden=true` / `draft` / `deleted` / `purged` は通報不可（`ErrTargetNotEligibleForReport` → Frontend `not_found`）。
smoke target が hidden=true の場合は `cmd/ops photobook unhide` で一時 unhide → smoke → `cmd/ops photobook hide` で再 hide のサイクルが必要（`erroneous_action_correction` → `policy_violation_other`）。
visibility は smoke のために変更しない方針（unlisted target をそのまま使うのが理想）。

設計判断: [`docs/plan/post-pr36-submit-report-visibility-decision.md`](../plan/post-pr36-submit-report-visibility-decision.md)

### 11.4 cleanup は「smoke 由来の行に限定」して TX で

DELETE 前に必ず `FOR UPDATE` で対象を lock し、rowcount が想定値と一致しなければ ROLLBACK。
削除対象:
- `reports`（status='submitted' AND target=smoke target AND submitted_at last 30min）
- `outbox_events`（event_type='report.submitted' AND status='pending' AND aggregate_id=smoke report_id）
- `usage_counters`（scope_type='source_ip_hash' AND action='report.submit' AND created_at last 1h）

PR35b 由来の `resolved_action_taken` 行などは保持。

### 11.5 cmd/ops 出力の redact 取り扱い

cmd/ops `[ok]` 行末尾に raw photobook_id が出ることがある（sed 後段だけに依存しない）。chat 一回限り表示は許容、**work-log / commit / failure-log には raw 値を残さない**。
...
| 2026-05-01 | §11「実機 429 smoke 注意」を追加。PR36 STOP ε で得た rollover / visibility / cleanup / redact 教訓を反映 |
| 2026-05-01 | §11.3 を `visibility != private` に更新（SubmitReport 緩和に追従、案 B 採用）|
