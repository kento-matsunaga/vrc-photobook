...
- **R2 の content-length-range / Content-Length 制約の実地検証**（M3 冒頭、または **R2 接続 PoC（M1 優先順位 4）で確認**）
- **R2 接続の最小動作確認**（Backend Cloud Run 相当から HeadBucket / ListObjects / presigned PUT 発行が動くか）— **M1 R2 接続 PoC で実施予定**。コード実装は 2026-04-25 完了（コミット `a33be4c`）。実 Cloudflare 接続検証は次工程の Wrangler 疎通 → S3 API の順で実施
- **HEIC デコードの image-processor コンテナ**（libheif + libde265 同梱、Cloud Run Jobs / Worker で動くか）— **R2 接続 PoC には含めず別 PoC または M3〜M6 で扱う**。理由: cgo + libheif の重い検討は R2 接続成立後に切り出す方が手戻りが少ない
...
### Turnstile + upload_verification_session（U5 解消、2026-04-25）

M1 優先順位 5 の PoC を `harness/spike/backend/` で完了（コミット `53fa568`）。本 ADR §Turnstile 検証 / §upload_verification_session の保存先 / 並列消費の競合解決に関わる U5 を解消した。

**確認済みの設計事実:**

- **mock モード**: `TURNSTILE_SECRET_KEY` が空のとき自動で mock 動作。`MOCK_FAIL` を含む token のみ拒否、それ以外は成功扱い。Cloudflare アカウント無しでもローカルで PoC が完結する設計を採用
- **実 Cloudflare 検証**: 公開サンドボックス secret（必ず success: `1x0000...AA` / 必ず failure: `2x0000...AA`）で `siteverify` を実呼び出しし、200 / 403 + サーバ側 `error_codes` ログを確認
- **DB 保存方式**: raw token は **クライアントへのレスポンスのみ**。DB は SHA-256（32 バイト bytea）のみを保存し、`encode(session_token_hash, 'hex')` の長さは常に 64 桁
- **アトミック消費**: 単一 SQL UPDATE の `WHERE` 句に 5 条件（`session_token_hash` + `photobook_id` + `revoked_at IS NULL` + `expires_at > now()` + `used_intent_count < allowed_intent_count`）を AND で並べ、`used_intent_count` を +1 して `RETURNING`。0 行返却（`pgx.ErrNoRows`）を 403 `consume_rejected` に集約
- **20 回上限の厳守**: 逐次 21 回 consume で `[1〜20] 200 / [21] 403` を確認
- **100 並列 race**: `scripts/turnstile-consume-race.sh` で 100 並列 consume を発射し `success=20 / forbidden=80 / その他=0` を確認。PostgreSQL Read Committed の単一行 UPDATE で原子性が保証される
- **拒否カテゴリの集約**: hash 不一致 / photobook_id 不一致 / 期限切れ / revoked / 残数枯渇 のいずれも 403 `consume_rejected` の単一カテゴリで返却。攻撃者は脱落条件を判別できない
- **漏洩防止**: secret / Turnstile token / verification_session_token がログに出ないことを `grep` で 0 ヒット確認

**本実装 (M2〜) への引き継ぎ:**

- DB スキーマは `harness/spike/backend/migrations/00002_create_upload_verification_sessions.sql` の最小スキーマを起点とし、auth/upload-verification 集約として再設計（ドメインモデル / VO / Repository を `domain-standard.md` の構造で整備）
- Atomic Consume の SQL は本実装でも踏襲（5 条件 AND の単一 UPDATE）。`RETURNING used_intent_count, allowed_intent_count` をユースケース層で消費後の残数算出に使う
